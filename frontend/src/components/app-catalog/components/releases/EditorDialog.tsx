...
export function EditorDialog(props: {
  ...
  const [versions, setVersions] = useState<
    Array<{ value: string; title: string; version: string }>
  >([]);
      ...
      const fetchChartVersions = async () => {
        ...
        const chartsCopy = _.cloneDeep(charts).sort((a: any, b: any) => {
          let compareValue = semver.compare(
            semver.coerce(a.version) as semver.SemVer,
            semver.coerce(b.version) as semver.SemVer
          );
          if (compareValue === 0) {
            ...
            {isUpdateRelease && (
              <Box ml={2}>
                <Autocomplete
                  ...
                  onChange={(
                    event,
                    newValue: { value: string; title: string; version: string } | null
                  ) => {
